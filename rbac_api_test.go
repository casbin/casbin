...
// TestUndefinedFunctionGWithLoadPolicy tests the scenario where:
// 1. An enforcer is created with a model and adapter
// 2. Grouping policies are added via API
// 3. LoadPolicy() is called (which creates a copy of the model)
// 4. Enforce is called and should work without "Undefined function g" error
func TestUndefinedFunctionGWithLoadPolicy(t *testing.T) {
	// Create a model
	text := `
[request_definition]
r = sub, obj, act
...
`
	...
	tmpFile := t.TempDir() + "/test_undefined_g_policy.csv"
	// Create the file
	if err := fileadapter.NewAdapter(tmpFile).SavePolicy(m); err != nil {
	...
	// Save the policies to file so they persist through LoadPolicy
	err = e.SavePolicy()
	if err != nil {
		t.Fatalf("SavePolicy failed: %v", err)
	}

	// Call LoadPolicy - this creates a copy of the model
	// If RM is not copied, this will cause the bug
	err = e.LoadPolicy()
	if err != nil {
		t.Fatalf("LoadPolicy failed: %v", err)
	}

	// Try to enforce - this should not give "Undefined function g" error
	res, err := e.Enforce("alice", "data1", "read")
	if err != nil {
		t.Fatalf("Enforce failed with error: %v (likely 'Undefined function g')", err)
	}

	if !res {
		t.Error("Expected alice to have read access to data1 via admin role, but got false")
	}
}

// TestUndefinedFunctionGWithModelCopy tests that copying a model preserves RM and CondRM
func TestUndefinedFunctionGWithModelCopy(t *testing.T) {
	// Create a model
	text := `
[request_definition]
r = sub, obj, act
...
`
	...
	// Create enforcer to initialize the role manager
	e, err := NewEnforcer(m)
	if err != nil {
		t.Fatal(err)
	}

	// Verify original model has RM set
	if ast, ok := e.GetModel()["g"]["g"]; !ok {
		t.Fatal("g assertion not found in model")
	} else if ast.RM == nil {
		t.Fatal("Original model's ast.RM is nil")
	}

	// Copy the model (simulating what LoadPolicy does)
	newModel := e.GetModel().Copy()

	// Verify copied model also has RM set
	if ast, ok := newModel["g"]["g"]; !ok {
		t.Fatal("g assertion not found in copied model")
	} else if ast.RM == nil {
		t.Error("Copied model's ast.RM is nil - this is the bug!")
	}
}
